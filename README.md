...
- `Space`：在当前格插入空位，将当前及后续连续内容右移到最近空格；右侧没有空格时不移动
- `Backspace`：当前格为空且右侧有内容时删除这个空位并将后续内容左移；右侧没有内容时返回上一格
- `←` / `→`：先在当前单词的字母间移动光标；到达开头或末尾后切换输入格
- `↑` / `↓`：按照页面实际换行位置，移动到正上方或正下方最接近的输入格；到达上下边界后不移动
- `Tab`：重播当前句并保留焦点
- `Esc`：停止播放

首页右上角的“键位设置”可以修改插入空格、下一格 / 检查答案、重新播放和停止播放键位；点击长方形按键框后直接按新键即可。设置仅保存在当前浏览器，四个方向键和空输入格的 `Backspace` 行为固定不变。
