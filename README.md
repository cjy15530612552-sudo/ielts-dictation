...
- `Enter`：移动到下一格；在最后一格检查当前整句答案
- `Space`：保留当前内容并移动到下一个可编辑输入格；当前格为空时也可前进，最后一格保持不动
- `Backspace`：当前格为空时返回上一个可编辑输入格，并把光标放到已有文本末尾；当前格有内容时正常删除字符
- `←` / `→`：先在当前单词的字母间移动光标；到达开头或末尾后切换输入格
- `↑` / `↓`：按照页面实际换行位置，移动到正上方或正下方最接近的输入格；到达上下边界后不移动
- `Tab`：重播当前句并保留焦点
- `Esc`：停止播放

首页右上角的“键位设置”可以修改切换到下一格、下一格 / 检查答案、重新播放和停止播放键位；点击长方形按键框后直接按新键即可。设置仅保存在当前浏览器，四个方向键和空输入格的 `Backspace` 行为固定不变。

答案页支持“上一句”“再听一次”“重新开始当前句”“下一句”；当前句重新开始会清空本句输入并自动重播。首页练习卡支持持久化的整套练习“重新开始”。

收藏单词时，后端会使用统一的 TTS 配置自动预生成并缓存单词发音。单词本中的喇叭按钮直接播放缓存音频；旧收藏或失败项目可点击喇叭重新生成。此能力适用于所有练习及未归类收藏。

单词本及任意练习分组均可进入单词听写：系统按当前列表顺序播放发音；选择“理解”会直接进入下一个词，选择“不理解”会先显示答案，再由用户继续。判断选项也支持数字键 `1`、`2`。
