...
const advanceKey = desktop.getByRole("button", { name: "切换格子键位，当前 Space" });
await advanceKey.click();
await advanceKey.press("n");
await desktop.getByText("切换格子已设为 N。").waitFor();
...
if (!(await second.evaluate((element) => element === document.activeElement))) {
  throw new Error("Custom advance key did not move to the next input");
}
if ((await first.inputValue()) !== "The") throw new Error("Custom advance key changed the answer");
